...
export function multiRoomCombinedFareBreakDown(selectedRooms, guests = 1) {
  ...
  const roomBreakdowns = Object.entries(groupByRoomType).reduce(
    (acc, [roomType, rooms]) => {
      const breakdowns = rooms.map((room) => {
        const breakdown = singleRoomFareBreakdown(room, 1);
        return breakdown;
      });
  ...
  // Calculate total by iterating through all room types and bed options
  let grandTotal = 0;
  for (const [roomType, bedOptions] of Object.entries(roomsByRoomType)) {
    for (const [bedOption, breakdown] of Object.entries(bedOptions)) {
      console.log(`Room Type: ${roomType}, Bed Option: ${bedOption}, Total: ${breakdown.total}`);
      grandTotal += breakdown.total || 0;
    }
  }
  combinedBreakdown.total = grandTotal;
