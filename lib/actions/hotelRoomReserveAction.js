...
export default async function hotelRoomReserveAction(bookingData) {
  ...
  const bookingObj = {
    ...
    rooms: selectedRooms
      .map((room) => {
        try {
          // Use originalId if available (from API), otherwise try _id
          const idToUse = room.originalId || room._id;
          
          // Check if it's already an ObjectId
          if (ObjectId.isValid(idToUse)) {
            return new ObjectId(idToUse);
          }
          
          // If not valid, log and skip
          console.warn("Skipping invalid room ID:", idToUse);
          return null;
        } catch (e) {
          console.error("Error converting room ID:", room._id, e);
          return null;
        }
      })
      .filter(id => id !== null),
