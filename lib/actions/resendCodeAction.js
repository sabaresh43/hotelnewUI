...
import { passwordResetVerificationEmailTemplate } from "../email/templates";
...
export default async function resendCodeAction() {
  ...
  if (!vdStr && e_iStr) {
    ...
    try {
      ...
      const htmlEmail = passwordResetVerificationEmailTemplate({
        ...emailDefaultData,
        main: {
          code: token.token,
          expirationTime: formatInTimeZone(
            token.expires,
            timeZone,
            "PPpp zzzz",
          ),
