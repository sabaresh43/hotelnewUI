    ...
    async authorize(credentials) {
      try {
        ...
        if (!userDetails || !Object.keys(userDetails).length) {
          console.log('Credentials authorize: user not found for', credentials.email);
          return null;
        }
      if (!userDetails || !Object.keys(userDetails).length) {
        return null;
      ...
      const { password } = await getOneDoc(
        ...
        console.log('Credentials authorize: password valid?', !!isValid, 'for', credentials.email);
        if (isValid) {
          return userDetails;
        }
        return null;
      } catch (err) {
        console.error('Credentials authorize error for', credentials.email, err);
        return null;
      }
    },
  };
